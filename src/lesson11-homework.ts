...
function exercise46() {
  ...
  // TODO: create a type that extracts the type of the first argument of a function FirstParamType
   type FirstParameter<T> = T extends (...args: infer Args) => any ? Args[0] : never; // use infer keyword
  ...
  // And creates a new function type definition with the same return type, and the first param only
   type FirstParameterFunction<T> = (arg: FirstParameter<T>) => MyReturnType<T>; // use infer keyword
...
function exerciseA() {
  ...
  function areAnagrams(s1: string, s2: string): boolean {
    const cleanString = (str: string) => str.replace(/[^a-zA-Z]/g, '').toLowerCase();
    const sortedS1 = cleanString(s1).split('').sort().join('');
    const sortedS2 = cleanString(s2).split('').sort().join('');
