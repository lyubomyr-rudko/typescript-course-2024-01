...
function exercise46() {
  ...
  type FirstParameter<T> = T extends (...args: infer R) => unknown
    ? R[0]
  ...
  type FirstParameterFunction<T> = T extends (...args: infer R) => infer A
    ? (firstName: R[0]) => A
    : never; // use infer keyword
...
function exerciseA() {
  ...
  console.log(areAnagrams('listen', 'silent')); // False
