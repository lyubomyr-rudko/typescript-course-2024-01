...
export function exercise27() {
    ...
    const humanInstance = new Human ('Ann', 12, 2);
    const animalInstance = new Animal ('Xan', 1, 'dog');

    function printPassangerInfo1(passanger: TPassanger) {
        console.log(passanger.name);
        console.log(passanger.age);
        // using instanceof operator to narrow the type of the passenger
        if (passanger instanceof Human) {
            const human = passanger as THuman;
            console.log('Driver License ID:', human.driverLicenseId);
        } else if (passanger instanceof Animal) {
            const animal = passanger as TAnimal;
            console.log('Species:', animal.species);
        }
    }
    printPassangerInfo1(humanInstance);
    printPassangerInfo1(animalInstance);
...
//   interface IUser{
//     address: IAddress; розширила interface IUser
//   }
...
//   interface IUser {
//     company: ICompany; розширила interface IUser
//   }
  ...
  function getUserNames(users: IUser[]): { firstName: string, lastName: string }[] {
  ...
  function companyNameCatch(users: IUser[]): string {
    let longestCatchPhrase = '';
    let companyName = '';
  
    users.forEach(user => {
      if (user.company.catchPhrase.length > longestCatchPhrase.length) {
        longestCatchPhrase = user.company.catchPhrase;
        companyName = user.company.name;
      }
    });
    return companyName;
  }
  console.log(companyNameCatch(users))
  
  // TODO: define a function that returns a list of users that have website ending with .org
  function getUserWithOrgWebSite(IUser:[]): IUser[]{
  ...
  function getUsersCities(users: IUser[]): string[] {
...
function getUserWithOrgWebSite(users: IUser[]): IUser[] {
    return users.filter(user => user.website.endsWith('.org'));
}
...
export { mockUsers, fetchUsers, getUserNames, getCompanyName, companyNameCatch, getUserWithOrgWebSite};
