...
function exercise31() {
  ...
  function handleSaveUserSubmitCallback(onSubmit: TSaveUserCallback) {
  ...
  function handleSaveUserSubmitInterface(onSubmit: ISaveUserCallback) {
    const firstName = 'John';
    const lastName = 'Smith';
  ...
  createForm(handleSaveUserSubmit as TSaveUserCallback);
  ...
  // TODO: remove type assertion, fix the error
  createForm2(handleSaveUserSubmit as TSaveUserCallback);

  // *** add constructor signatures here ***
  type TUserConstructor = new (name: string) => any;
...
function exercise34() {
  ...
  let studentWithHighestAverageGrade = '';
  ...
  Object.keys(studentGrades).forEach((studentName) => {
    ...
    if (typeof averageGrade === 'number' && averageGrade > highestAverageGrade) {
      highestAverageGrade = averageGrade;
      studentWithHighestAverageGrade = studentName;
    }
  });
