...
export function excercise13A() {
  ...
  return { getRandomNumber, getRandomString, getRandomObject, getRandomItem };
...
console.log('Chunked Array 1:', result1);

const result2 = chunkArray([1, 2, 3, 4, 5, 6, 7, 8, 9], 3);
console.log('Chunked Array 2:', result2);
...
function excercise15() {
  function fetchUserAge(): number {
    const responseText = '{"name": "John", "age": "16"}'; // Convert age to number
    // we can use parseInt(JSON.parse(responseText).age, 10) to parse as an integer.
    return JSON.parse(responseText).age;
  }
