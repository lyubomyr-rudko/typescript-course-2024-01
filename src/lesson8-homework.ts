...
function exercise35() {
  ...
  let user: TUser = { userName: 'Artem', title: 'QA', email: 'test@gmail.com' };
  let product: TProduct = { title: 'apple', price: 1, quantity: 10 };
...
function exercise36() {
  ...
  function toString(this: { age: number; role: string; name: string }) {
...
function exercise38() {
  ...
  if (fetchResult !== null) {
    ...
    userNames.forEach((name) => {
      ...
      //сужение типов не срабатывает так как мы передем в forEach колюэк функцию,
      //которая по - умолчанию тайпскриптом воспринимается как потенциальный нул или
      //массив объектов.И в колбэке тайпскрип не может определить, что именно тут на данный момент
      // TODO: remove this line
...
function exercise39() {
  ...
  function printAllUserPropTypes(obj: typeof user): void {
  ...
  function getUserAddress(obj: typeof user): typeof user.address.address {
