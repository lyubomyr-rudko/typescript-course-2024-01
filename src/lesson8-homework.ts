...
function exercise36() {
  ...
  // console.log(data + ''); // Calls toString() implicitly
  console.log(data.toString());

  // Update the data object to include required properties
  const updatedData = {
    firstName: 'Joe',
    age: 30,
    role: 'Developer',
  ...
  // console.log(updatedData + ''); // Calls toString() implicitly
...
function exercise38() {
  ...
  const fetchResult: GroupDocument[] | null = [
    {
      name: 'John',
      data: [
        {
...
function exercise39() {
  ...
  function getCoordinates() {
    return user.company?.address?.coordinates || null;
  }
...
console.log(getIntersection([8, 3, 2, 4, 2], [7, 3, 4, 5, 3]));
