...
function exercise40() {
  ...
  //   descriptor.value = function replacementMethod(
  //     this: unknown,
  //     ...args: number[]
  //   ) {
  //     calls++;
  ...
  //     console.log(`${key} = call count ${calls}`);
  //     return result;
  //   };
  ...
  //     const result = originalMethod.call(this, ...args);
  //     console.log(`${key} = execution time ${timeAfter - timeBefore}`);
  //     return result;
  //   };
...
function exercise41() {
  ...
  function callCount(
    originalMethod: (...args: number[]) => number,
    _context: TDecoration,
  ) {
    let count = 0;
    function replacementMethod(this: unknown, ...args: number[]) {
      count++;
      const result = originalMethod.call(this, ...args);
      console.log(` ${_context.name}= call count  ${count}`);
      return result;
    }
  ...
  function executionTime(
    ...
    function replacementMethod(this: Calculation, ...args: number[]) {
      console.time(_context.name);
      const result = originalMethod.apply(this, args);
      console.timeEnd(_context.name);
      return result;
    }
    return replacementMethod;
  }
