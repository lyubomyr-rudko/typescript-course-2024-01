...
function exercise40() {
  ...
  function TimeExecution(
    ...
    descriptor.value = function (...args: number[]) {
      ...
      const result = originalMethod.apply(this, args);
      console.log(
        `Time of the function execution is ${new Date().getTime() - date} `,
      );
      return result;
    };
  }
  ...
  const calc1 = Calculation.add(1, 2);
  const calc2 = Calculation.add(1, 4);
...
function exercise41() {
  ...
  function CallCount(
    ...
    function replacementMethod(this: Calculation, ...args: number[]) {
      calls += 1;
      const result = originalMethod.apply(this, args);
      console.log(`${String(context.name)} method was called ${calls} times`);
      return result;
    }
  ...
  function TimeExecution(
    ...
    function replacementMethod(this: Calculation, ...args: number[]) {
      ...
      const result = originalMethod.apply(this, args);
      console.log(
        `Time of ${String(context.name)} method execution is ${
