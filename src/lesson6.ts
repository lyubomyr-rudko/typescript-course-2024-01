...
function assertionFunctions() {
  ...
  assert(person !== undefined, 'person must be defined');
...
function implementsKeyword() {
  ...
  const anotherPerson = new Personv2('John', 18);
  console.log(anotherPerson.name, anotherPerson.age);
  ...
  class Person3 implements TName, TAge {
    name: string;
    age: number;
  ...
  const person3 = new Person3('John', 18);
...
function interfacesVsTypeAliases() {
  ...
  interface ISetPointFn {
    (x: number, y: number): IPoint;
  }

  const fn: ISetPointFn = (x, y) => {
    return { x, y };
  };
  const point: IPoint = fn(1, 2);
  console.log(point);
  ...
  type TSetPoint = (x: number, y: number) => TPoint;
  const fn2: TSetPoint = (x, y) => {
    return { x, y };
  };
  const point2: TPoint = fn2(1, 2);
  console.log(point2);
  ...
  //   class SomePoint implements PartialPoint {
  //     x = 1;
  //     y = 2;
  //   }
  // tuple
  type Data = [number, string];
  const data: Data = [1, 'abc'];
  console.log(data);
  //   class SomeData implements Data { // error
  //     0 = 1;
  //     1 = 'abc';
  //   }
  ...
  //   type TPoint = IPartialPointX & { y: number };
  // Interface extends type alias
  interface IPoint extends TPartialPointX {
    y: number;
  }
